...
echo "Granting service principal permissions to the '${storageAccount}' Blob storage."
az role assignment create --role "Storage Blob Data Contributor" --assignee "${clientId}" --scope "/subscriptions/${subscription}/resourceGroups/${resourceGroupName}/providers/Microsoft.Storage/storageAccounts/${storageAccount}" 1>/dev/null
